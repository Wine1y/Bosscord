...
class Fighter():
    ...
    @classmethod
    def from_boss(cls, boss: Boss) -> "Fighter":
        calculator = BossCalculator(boss)
        return cls(
            boss, calculator.health, calculator.health, calculator.attack, calculator.armor,
            calculator.crit_chance,calculator.crit_damage, calculator.skill, calculator.pet,
            calculator.items, None
        )
...
class FightStatus():
    ...
    def create_boss_fight(
        ...
        user_fighter = Fighter.from_user(user)
        boss_fighter = Fighter.from_boss(boss)

        user_fighter.enemy = boss_fighter
        boss_fighter.enemy = user_fighter

        fighters = [user_fighter, boss_fighter]
    ...
    def to_embed(self, winner: Optional[Fighter]=None) -> Embed:
        ...
        if winner is not None:
            ...
            if type(winner.profile) is Boss and winner.profile.image is not None:
                embed.set_thumbnail(winner.profile.image)
            elif type(winner.profile) is Row:
        ...
        if type(current_fighter.profile) is Boss:
            turn_image = current_fighter.profile.image
        else:
...
class BossFight():
    ...
    def __init__(self, user: Row, boss: Boss, client: Bot, interaction: Interaction) -> None:
        self.fight_status = FightStatus.create_boss_fight(user, boss, interaction, client)
